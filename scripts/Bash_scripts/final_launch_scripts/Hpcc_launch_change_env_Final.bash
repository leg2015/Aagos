...
-OUTPUT_DIR) # different from DATA_FILEPATH of aagos world. that is the full path to where the data files should be stored. this val is what to name the home dir, NOT a filepath!!
OUTPUT_DIR="$2"
...
-VARIABLES) #TODO: currently must be an absolute path, would be nice to fix that
VARIABLES="$2"
shift # shift past curr argument
shift # shift past curr value
;; # indicates end of case
...
# //TODO: move into aagos directory, make sure ../Empirical is in that direceory, may break things??
cd $AAGOS_PATH
if [[ ! -d  ../Empirical ]]; then
...
while [[ \$INDEX -lt $NUM_REPLICATES ]]
    do
        ...
        time ./Aagos -DATA_FILEPATH \$FILE_PATH $CURR_PARAMS -CHANGE_RATE ${CHANGE_TO_TRY[$IND]}
        let INDEX=INDEX+1
    done
EOF

...
## Coding sites by change rate

### Coding sites by change rate - Gradient fitness model

```{r}
gradient_model <- 1
phase <- 0
gen <- 50000
data <- filter(agg_data, GRADIENT_MODEL == gradient_model & 
                         evo_phase == phase & 
                         update == gen)
ggplot(data,
...
```{r}
kruskal.test(formula = coding_sites ~ chg_rate_label, data=data)
pairwise.wilcox.test(x=data$coding_sites,
                     g=data$chg_rate_label,
                     exact=FALSE,

---
title: "2020-05-30 - Gene Moves and Insertion/Deletion Mutations - Experiment Analysis"
output:
  html_document:
    keep_md: no
    toc: true
...
agg_data$BIT_INS_PROB <- as.factor(agg_data$BIT_INS_PROB)
agg_data$BIT_DEL_PROB <- as.factor(agg_data$BIT_DEL_PROB)
agg_data$GENE_MOVE_PROB <- as.factor(agg_data$GENE_MOVE_PROB)
...
ggplot(data_gradient_phase_0, 
       aes(x=chg_rate_label, y=coding_sites, color=GENE_MOVE_PROB)) +
  geom_boxplot() +
  ylim(0, 150) +
  facet_grid(BIT_FLIP_PROB~BIT_INS_PROB, scales="free_y", labeller = label_both) +
...
a <-
ggplot(data_gradient_phase_0, 
       aes(x=chg_rate_label, y=genome_length, color=GENE_MOVE_PROB)) +
  geom_boxplot() +
  ylim(0, 1025) +
  facet_grid(BIT_FLIP_PROB~BIT_INS_PROB, scales="free_y", labeller = label_both) +
  ggtitle("Gradient Fitness Model") +
...
a <-
ggplot(data_gradient_phase_0, 
       aes(x=chg_rate_label, y=fitness, color=GENE_MOVE_PROB)) +
  geom_boxplot() +
  ylim(0, 17) +
  facet_grid(BIT_FLIP_PROB~BIT_INS_PROB, scales="free_y", labeller = label_both) +
  ggtitle("Gradient Fitness Model") +
...
We lock down the genetic architecture and evolve in a new (random), static environment.
...
ggplot(data_gradient_phase_1, 
       aes(x=chg_rate_label, y=fitness, color=GENE_MOVE_PROB)) +
  geom_boxplot() +
  facet_grid(BIT_FLIP_PROB~BIT_INS_PROB, scales="free_y") +
  ggtitle("Gradient Fitness Model") +
...
a <-
ggplot(data_gradient_phase_1, 
       aes(x=coding_sites, y=fitness, color=chg_rate_label, shape=GENE_MOVE_PROB)) +
  geom_point() +
  facet_grid(BIT_FLIP_PROB~BIT_INS_PROB, scales="free") +
  ggtitle("Gradient Fitness Model") +
  theme(legend.position = "top") +
  theme(panel.background = element_rect(fill = "grey90"))

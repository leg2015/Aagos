...
def main():
    ...
    nk_combos = [f"{chg} {mut} {genome} {moves} {indels} -GRADIENT_MODEL 0" for chg in nk_config["environment_change"] for mut in shared_config["BIT_FLIP_PROB"] for genome in shared_config["GENOME_SIZE"] for moves in shared_config["GENE_MOVES"] for indels in shared_config["INDELS"]]
    gradient_combos = [f"{chg} {mut} {genome} {moves} {indels} -GRADIENT_MODEL 1" for chg in gradient_config["environment_change"] for mut in shared_config["BIT_FLIP_PROB"] for genome in shared_config["GENOME_SIZE"] for moves in shared_config["GENE_MOVES"] for indels in shared_config["INDELS"]]
    ...
    with open("arch_muts_resub.sb", "w") as fp:
        fp.write(script)

...
def main():
    ...
    nk_combos = [f"{chg} {mut} {genome} -GRADIENT_MODEL 0" for chg in nk_config["environment_change"] for mut in shared_config["BIT_FLIP_PROB"] for genome in shared_config["TOURNAMENT_SIZE"] ]
    gradient_combos = [f"{chg} {mut} {genome} -GRADIENT_MODEL 1" for chg in gradient_config["environment_change"] for mut in shared_config["BIT_FLIP_PROB"] for genome in shared_config["TOURNAMENT_SIZE"] ]
    ...
    with open("num_bits_resub.sb", "w") as fp:
        fp.write(script)

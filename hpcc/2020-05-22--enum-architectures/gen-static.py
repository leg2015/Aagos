...
def main():
    ...
    parser.add_argument("--arch_dir", type=str, help="Where should we pull architectures from?")
    ...
    # grab all architectures
    architectures = [os.path.join(arch_dir, arch) for arch in os.listdir(arch_dir) if "architecture-" in arch]
    ...
    for condition_id in range(0, len(combos)):
        ...
        for i in range(1, num_replicates+1):
            ...
            run_name_params = " ".join(condition_params.split(" ")[:-2]) + f" -SEED {seed}" # chop of arch directory path at end
            run_name = ("RUN " + run_name_params).replace("-", "_").replace(" ", "_")
